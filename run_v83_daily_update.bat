...
set "LOG=logs\v83_daily_update.log"
echo [INFO] Running MONE v83 daily update... > "%LOG%"
...
%PYEXE% run_v83_daily_update.py >> "%LOG%" 2>&1
set "ERR=%ERRORLEVEL%"
type "%LOG%"
if not "%ERR%"=="0" (
  echo.
  echo [ERROR] MONE v83 update failed. See %LOG%
  pause
  exit /b %ERR%
)
echo [OK] MONE v83 daily update complete.
pause
