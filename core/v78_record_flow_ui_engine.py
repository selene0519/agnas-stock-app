...
US_TICKER_RE = re.compile(r'^[A-Z]{1,6}(?:[.-][A-Z]{1,3})?$')
...
COMMON_US = {
    ...
    'DDOG','CRWD','S','NET','RIOT','SOXX','RKLB','LUNR','IONQ','OKLO','HUT','IREN'
}
...
def _read_csv_any(path: str | Path) -> pd.DataFrame:
    p = Path(path)
    # v78 fix: absolute/data/history paths must not be silently prefixed with reports/.
...
def _symbols(row: pd.Series) -> list[str]:
    vals = []
    for c in ['종목코드','종목','symbol','ticker','티커','코드','TOP','카드제목','종목명','name']:
        if c in row.index and _clean(row.get(c)):
            vals.append(_clean(row.get(c)))
    text = ' '.join(vals)
    syms = [s.upper() for s in re.findall(r'\b[A-Z]{1,6}(?:[.-][A-Z]{1,3})?\b', text)]
    for v in vals:
        z = _zcode(v)
...
def _is_us_row(row: pd.Series) -> bool:
    text = _row_text(row)
    syms = _symbols(row)
    if any(w.lower() in text.lower() for w in US_WORDS):
        return True
    return any(s in COMMON_US or US_TICKER_RE.fullmatch(s) for s in syms)
...
def _is_kr_row(row: pd.Series) -> bool:
    ...
    if any(w.lower() in text.lower() for w in KR_WORDS):
        return True
    if any(KR_CODE_RE.fullmatch(_zcode(s)) for s in syms):
        return True
    # Korean name with no US ticker generally indicates KR row.
    return bool(HANGUL_RE.search(text)) and not _is_us_row(row)


def _row_ok(row: pd.Series, slug: str) -> bool:
    if slug == 'kr':
        return _is_kr_row(row) and not _is_us_row(row)
    if slug == 'us':
        return _is_us_row(row) and not (_is_kr_row(row) and not _is_us_row(row))
    return True
...
def _pick(names: list[str], slug: str) -> pd.DataFrame:
    for name in names:
        df = _read_report(name.format(slug=slug))
        if not df.empty:
            return df
    return pd.DataFrame()
...
def _normalize_flow_rows(df: pd.DataFrame, slug: str, source: str) -> pd.DataFrame:
    if df is None or df.empty:
        return pd.DataFrame()
    # Use internal content, not filename, to split KR/US.
    if slug == 'kr':
        rows = df[[bool(_is_kr_row(r) and not _is_us_row(r)) for _, r in df.iterrows()]].copy()
    else:
        rows = df[[bool(_is_us_row(r)) for _, r in df.iterrows()]].copy()
    if rows.empty:
        return pd.DataFrame()
    out_rows: list[dict[str, Any]] = []
    for _, r in rows.iterrows():
        raw_symbol = _first_nonempty(r, ['종목코드','symbol','ticker','종목','코드'])
        code = _zcode(raw_symbol) if slug == 'kr' else raw_symbol.upper().strip()
        name = _first_nonempty(r, ['종목명','name','종목','symbol','ticker']) or code
        last = _first_nonempty(r, ['현재가','last_price','current_price','price','close','직전종가'])
        chg = _first_nonempty(r, ['등락률','intraday_change_pct','change_pct','pct_change'])
        vol = _first_nonempty(r, ['거래량','intraday_volume','volume'])
        val = _first_nonempty(r, ['거래대금','intraday_trading_value','trading_value','amount'])
        score = _first_nonempty(r, ['수급점수','intraday_flow_score','intraday_momentum_score','sector_intraday_strength_score'])
        foreign = _first_nonempty(r, ['foreign_net_buy','외국인순매수','외국인'])
        inst = _first_nonempty(r, ['institution_net_buy','기관순매수','기관'])
        program = _first_nonempty(r, ['program_net_buy','프로그램'])
        score_num = _to_num(score)
        if math.isnan(score_num):
            # 거래량/거래대금 fallback: keep rows but label as fallback instead of dropping all KR rows.
            score_num = 50.0
        기준 = '외국인·기관 수급' if (foreign or inst or program) else '거래대금·거래량 보강'
        out_rows.append({
            '종목코드': code,
            '종목명': name,
            ...
            '거래량': vol or '-',
            '거래대금': val or '-',
            '수급점수': round(score_num, 2),
            '해석': '수급값 부족 시 거래량·거래대금 기준으로 보강했습니다.' if 기준.startswith('거래') else '외국인·기관·프로그램 흐름을 확인합니다.',
            '다음 행동': '장 시작 후 거래대금 증가와 기준가 근접 여부 확인',
            '데이터출처': source,
        })
    out = pd.DataFrame(out_rows).drop_duplicates(subset=['종목코드'], keep='first')
    if not out.empty:
        out = out.sort_values('수급점수', ascending=False).head(12).reset_index(drop=True)
    return out


def _build_flow(slug: str) -> tuple[pd.DataFrame, pd.DataFrame]:
    candidates = [
        f'v77_flow_clean_{slug}.csv', f'v76_flow_clean_{slug}.csv', f'v75_flow_clean_{slug}.csv', f'v74_flow_clean_{slug}.csv',
        f'v67_flow_{slug}.csv', 'intraday_flow_snapshot-Kang.csv', 'intraday_flow_snapshot-kang.csv', 'intraday_flow_snapshot.csv',
        'intraday_realtime_snapshot-Kang.csv', 'intraday_realtime_snapshot-kang.csv', 'intraday_realtime_snapshot.csv',
        'intraday_orderbook_snapshot-Kang.csv', 'intraday_orderbook_snapshot-kang.csv', 'intraday_orderbook_snapshot.csv',
        'intraday_data_coverage_diagnosis-Kang.csv', 'intraday_data_coverage_diagnosis.csv',
    ]
    picked = []
    diag = []
    for name in candidates:
        df = _read_report(name)
        if df.empty:
            continue
        norm = _normalize_flow_rows(df, slug, name)
        diag.append({'파일': name, '원본행수': len(df), '선택시장행수': len(norm), '시장': '한국주식' if slug == 'kr' else '미국주식'})
        if not norm.empty:
            picked.append(norm)
    if picked:
        out = pd.concat(picked, ignore_index=True).drop_duplicates(subset=['종목코드'], keep='first')
        out = out.sort_values('수급점수', ascending=False).head(12).reset_index(drop=True)
    else:
...
def _company(slug: str) -> pd.DataFrame:
    df = _pick(['v77_company_clean_{slug}.csv','v76_company_clean_{slug}.csv','v75_company_clean_{slug}.csv','v74_company_clean_{slug}.csv','v70_company_cards_{slug}.csv','v69_company_cards_{slug}.csv','operational_financial_kpi_{slug}.csv'], slug)
    return _clean_market_df(df, slug)


def _frames(slug: str) -> tuple[dict[str, pd.DataFrame], pd.DataFrame]:
    action = _clean_market_df(_pick(['v77_action_clean_{slug}.csv','v76_action_clean_{slug}.csv','v75_action_clean_{slug}.csv','v74_action_clean_{slug}.csv','v67_action_board_{slug}.csv','swing_candidates_{slug}_A_top3.csv'], slug), slug)
    pull = _clean_market_df(_pick(['v77_pullback_clean_{slug}.csv','v76_pullback_clean_{slug}.csv','v75_pullback_clean_{slug}.csv','v74_pullback_clean_{slug}.csv','v67_pullback_{slug}.csv','swing_candidates_{slug}_B_watch.csv'], slug), slug)
    flow, flow_diag = _build_flow(slug)
    risk = _clean_market_df(_pick(['v77_risk_clean_{slug}.csv','v76_risk_clean_{slug}.csv','v75_risk_clean_{slug}.csv','v74_risk_clean_{slug}.csv','v67_risk_{slug}.csv','v52_buy_risk_light.csv'], slug), slug)
    company = _company(slug)
    frames = {k: _ensure_schema(v, k, slug) for k, v in {'action':action, 'pullback':pull, 'flow':flow, 'risk':risk, 'company':company}.items()}
...
def _positions(slug: str) -> pd.DataFrame:
    df = _pick(['v77_position_cards_{slug}.csv','v76_position_cards_{slug}.csv','v75_position_cards_{slug}.csv','v74_position_cards_{slug}.csv','v67_position_plan_{slug}.csv','v50_position_plan.csv'], slug)
    return _clean_market_df(df, slug)
...
def _summary(slug: str, frames: dict[str, pd.DataFrame]) -> pd.DataFrame:
    return pd.DataFrame([
        {'카드':'오늘 우선 확인','아이콘':'🎯','설명':'기준가·손절가·목표가','건수':len(frames['action']),'TOP':_first(frames['action']),'상세파일':f'v78_action_clean_{slug}.csv','우선순위':1,'구분':'buy'},
        {'카드':'눌림목 진입 후보','아이콘':'🪜','설명':'추격보다 기준가 대기','건수':len(frames['pullback']),'TOP':_first(frames['pullback']),'상세파일':f'v78_pullback_clean_{slug}.csv','우선순위':2,'구분':'buy'},
        {'카드':'수급 급증 후보','아이콘':'💚','설명':'수급·거래대금 흐름','건수':len(frames['flow']),'TOP':_first(frames['flow']),'상세파일':f'v78_flow_clean_{slug}.csv','우선순위':3,'구분':'buy'},
        {'카드':'실적·저평가 후보','아이콘':'💎','설명':'재무·KPI·밸류','건수':len(frames['company']),'TOP':_first(frames['company']),'상세파일':f'v78_company_clean_{slug}.csv','우선순위':4,'구분':'value'},
        {'카드':'매수금지·주의','아이콘':'🚫','설명':'제외·관망 우선','건수':len(frames['risk']),'TOP':_first(frames['risk']),'상세파일':f'v78_risk_clean_{slug}.csv','우선순위':5,'구분':'risk'},
    ])
...
def _market_guard(slug: str, frames: dict[str, pd.DataFrame], sector: pd.DataFrame) -> pd.DataFrame:
    ...
    return pd.DataFrame([
        ...
        {'항목':'수급·거래대금 후보','값':f"{len(frames['flow'])}개",'점수':min(100, len(frames['flow'])*10),'다음 행동':'거래대금 확인'},
...
def _future(slug: str, frames: dict[str, pd.DataFrame]) -> pd.DataFrame:
    rows = []
    for kind, label, base_prob in [('action','오늘 확인',58),('pullback','눌림목',55),('flow','수급·거래대금',57),('company','실적·저평가',54),('risk','주의/제외',42)]:
...
def _history_rows_for(slug: str, frames: dict[str, pd.DataFrame]) -> pd.DataFrame:
...
def _append_history_all(new_frames: list[pd.DataFrame]) -> int:
    HISTORY_DIR.mkdir(parents=True, exist_ok=True)
    path = HISTORY_DIR / 'prediction_history.csv'
    new = pd.concat([x for x in new_frames if x is not None and not x.empty], ignore_index=True) if any(x is not None and not x.empty for x in new_frames) else pd.DataFrame()
    if new.empty:
        if not path.exists():
            pd.DataFrame(columns=['date','updated_at','market','category','symbol','name','base_price','stop','target','decision']).to_csv(path, index=False, encoding='utf-8-sig')
        return 0
    old = _read_csv_any(path) if path.exists() else pd.DataFrame()
...
def _data_status(slug: str, frames: dict[str, pd.DataFrame], news: pd.DataFrame, pos: pd.DataFrame, flow_diag: pd.DataFrame) -> pd.DataFrame:
    ...
    flow_reason = '국장 원본 수급 파일은 있으나 정제 결과가 없으면 거래대금·거래량 fallback을 사용합니다.' if slug == 'kr' else '미장은 실시간 수급 대신 모멘텀·거래대금 proxy를 사용합니다.'
    base = pd.DataFrame([
        {'시장':label,'항목':'오늘 후보','행수':len(frames['action']),'상태':'정상' if len(frames['action']) else '비어 있음','원인/설명':'기준가·손절가·목표가 후보'},
        {'시장':label,'항목':'수급·거래대금','행수':len(frames['flow']),'상태':'정상' if len(frames['flow']) else '적음/없음','원인/설명':flow_reason},
    ...
    if flow_diag is not None and not flow_diag.empty:
        diag = flow_diag.copy(); diag.insert(0, '시장구분', label)
        _write_csv(diag, f'v78_flow_source_diagnosis_{slug}.csv')
    return base


def run_v78_update(fetch_news: bool = True, fetch_fundamentals: bool = True, fetch_macro: bool = True) -> dict[str, Any]:
    _ensure_dotenv()
    base = {'status':'SKIPPED'}
    ...
    status_parts = []
    history_parts = []
    result: dict[str, Any] = {'status':'OK','version':'v78','updated_at':_now(),'base':base}
    for slug in ['kr','us']:
        frames, flow_diag = _frames(slug)
        news = _news_filter(_pick(['v77_news_cards_{slug}.csv','v76_news_cards_{slug}.csv','v75_news_cards_{slug}.csv','v74_news_cards_{slug}.csv','v70_news_cards_{slug}.csv','gnews_latest_{slug}.csv'], slug), slug)
        pos = _positions(slug)
        ...
        for key, df in frames.items():
            _write_csv(df, f'v78_{key}_clean_{slug}.csv')
        _write_csv(summary, f'v78_today_summary_{slug}.csv')
        _write_csv(news, f'v78_news_cards_{slug}.csv')
        _write_csv(pos, f'v78_position_cards_{slug}.csv')
        _write_csv(sector, f'v78_sector_strength_{slug}.csv')
        _write_csv(guard, f'v78_market_guard_{slug}.csv')
        _write_csv(future, f'v78_future_probability_{slug}.csv')
        q = _quant_outputs(slug, frames, pos)
        for k, df in q.items(): _write_csv(df, f'v78_quant_{k}_{slug}.csv')
        _write_csv(q['monte_carlo'], f'v78_monte_carlo_{slug}.csv')
        history_parts.append(_history_rows_for(slug, frames))
        result[slug] = {k: len(v) for k, v in frames.items()} | {'summary':len(summary),'news':len(news),'positions':len(pos),'sector':len(sector),'future':len(future)}
        status_parts.append(_data_status(slug, frames, news, pos, flow_diag))
    appended = _append_history_all(history_parts)
    result['history_appended_total'] = appended
    status = pd.concat(status_parts, ignore_index=True) if status_parts else pd.DataFrame()
    _write_csv(status, 'v78_data_status.csv')
    out = {'status':'OK','version':'v78','updated_at':_now(),'result':result,'env':{k: bool(_env(k)) for k in ['DART_API_KEY','FINNHUB_API_KEY','GNEWS_API_KEY','APIFY_TOKEN','SEC_USER_AGENT']}}
    _write_json(out, 'v78_status.json')
    return out


if __name__ == '__main__':
    print(json.dumps(run_v78_update(), ensure_ascii=False, indent=2, default=str))
