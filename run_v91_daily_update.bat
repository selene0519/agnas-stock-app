...
set "LOG=logs\v91_daily_update.log"
echo [INFO] Running MONE v91 integrated update... > "%LOG%"
...
%PYEXE% run_v91_daily_update.py >> "%LOG%" 2>&1
set "ERR=%ERRORLEVEL%"
type "%LOG%"
if not "%ERR%"=="0" (
  echo.
  echo [ERROR] MONE v91 update failed. See %LOG%
  pause
  exit /b %ERR%
)
echo [OK] MONE v91 daily update complete.
pause
