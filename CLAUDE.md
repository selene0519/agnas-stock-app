...
7. 🔶 **[부분 완료] 나머지 UI/모바일 폴리시** — 4개 주요 화면은 #6, 12개 화면 자동 계측은 07-29(3), 색 대비는 07-29(10), **관리자 화면(운영·자가보정 2탭)은 07-31**에 해결(로그인 게이트 때문에 그 전엔 한 번도 측정된 적이 없었다). **남은 것**: 백엔드를 띄운 상태의 journal(44px미달 29·기계코드 181)·news(기계코드 172)·home/chart(각 1) — 종전 "0건"은 백엔드 미기동 상태로 잰 결과였다. 그 외 실기기 시각 QA.
...
## 2026-07-31 — 관리자 화면은 세 번의 감사에서 **한 번도 측정된 적이 없었다**
07-29(3)에서 "12개 전 화면 실측 → 위반 0"이라 적었지만, `app/page.tsx:322`가
adminToken 없으면 AdminLoginPage를 그린다. 계측 스크립트는 토큰을 안 넣으므로
**세 번(07-26·07-28·07-29) 모두 로그인 폼을 재고 "0"이라 보고**했다. `admin`이
PAGES 목록에 **있었기 때문에** 아무도 빠진 걸 몰랐다 — 목록에 이름이 있는 것과
그 화면이 실제로 그려진 것은 다르다. 07-29(10)의 노트("broker/admin은 로그인
게이트라 본문이 140자")가 이미 단서였는데 렌더 판정 문제로만 읽고 넘어갔다.

토큰을 넣고 재니(로컬 백엔드 + `MONE_MEASURE_ADMIN_TOKEN`) **44px 미달 18건 /
기계코드 9건**. 자가보정 탭은 URL이 없어 어떤 pageId로도 안 닿았다.

**최악은 터치타깃이 아니라 안 보이는 버튼이었다.** 관리자 1차 액션
"GitHub 동기화"가 다크에서 **1.08:1**, 라이트에서 3.23:1이었다 — 사실상 투명.
원인은 AdminPage가 아니라 `globals.css`의 선택자다:

    .mone-app-shell [class*="bg-blue-"][class*="/"] { background: var(--brand-dim) }

`[class*="/"]`는 class 문자열 **어디의** 슬래시든 잡는다. `bg-blue-600`은 위
규칙에서 브랜드 배경 + 어두운 잉크(#052420)를 받는데, 같은 문자열에
`border-blue-500/30`이 있으면 이 규칙이 **더 구체적이라** 배경만 10% 틴트로
덮어쓴다 → 잉크만 남아 글자가 사라진다. 라이트 테마엔 같은 함정이 하나 더
있었다(`[class*="bg-blue-"][class*="/"][class*="text-white"]` → #0d9488).
**레포 전체에서 이 조합을 쓰는 곳은 AdminPage 단 한 곳**이었고, 그래서 계측이
닿지 않는 화면에 정확히 숨어 있었다. 선택자를 `bg-blue-<숫자>/` 형태로 좁혔다.

**고친 것:** 터치타깃 18건 → 0(공용 `ActionBtn`·헤더·시장/전략 토글·`<summary>`).
기계코드는 화면마다 자체 매핑을 두는 대신 공용 `statusLabel()`에 운영 코드
어휘를 더해 재사용(UNKNOWN·NORMAL·EMPTY_RESULT·LOADING·VERIFIED_90). 원문 코드는
관리자에게 진단 가치가 있으므로 **본문이 아니라 `title`**에 남긴다.
`FAILURE_REASON_LABELS`는 VirtualJournalPage 지역 상수였는데 자가보정 탭이 같은
코드를 원문으로 찍고 있어 `lib/statusLabels.ts`로 올려 공유했다.

**기능 버그도 하나 나왔다:** 결과 배너가 **운영 탭 블록 안에만** 있어서,
자가보정 탭의 "보정 파라미터 재계산"은 `setMessage`를 불러도 화면에 아무것도
안 떴다 — 누른 사람은 성공/실패를 알 수 없었다. 배너를 탭 밖으로 올리고
`role="status"` + 닫기 버튼 접근이름을 붙였다.

**계측기를 못 믿는 게 이번에도 맞았다.** 스크래치 프로브가 disabled 버튼을
4.11:1로 신고했는데, 레포의 `measure_frontend_contrast.py`는 이미
`:disabled`를 제외하고 있었다(WCAG 대상 외). 내 프로브만 틀렸다.

**회귀 방지(`tests/test_admin_screen_ui.py`) — 테스트를 세 번 고쳐야 했다:**
① 정적 Tailwind 검사로 버튼 높이를 재려다 배지·상태칩까지 7건 오탐 →
07-29(3)에서 "과탐 감시는 없느니만 못하다"고 폐기한 그 실수라, `<button>` 여는
태그에 붙은 class만 보게 좁혔다. ② 옛 선택자 문자열 검사가 **그 규칙을 설명한
내 주석**을 잡았다(이 레포에서 세 번째다) → 주석 제거 후 검사. ③ 충돌 검사를
`className=`으로만 짰는데 **실제 버그는 변형 맵(`const cls = {blue: "..."}`)
안에 있었다** — 즉 내 테스트는 방금 고친 그 버그를 못 잡았다. 문자열 리터럴
전체로 넓히고, 버그를 **일부러 다시 심어** 4개 검사가 전부 잡는 걸 확인했다.

**계측기 자체를 고쳤다(이게 본질):** `measure_frontend_touch_targets.py`에
`--admin-token`/`MONE_MEASURE_ADMIN_TOKEN` + 탭 드릴(`TAB_DRILLS`) 추가. 토큰
없이 돌리면 리포트에 `measuredAuthenticated=false`를 남기고 **"관리자 대시보드
본문은 미계측"이라고 경고를 찍는다** — 0을 통과로 오독하는 걸 막는 게 핵심이다.
탭만 열면 빈 상태(6건)를 재므로 데이터를 불러온 뒤 재도록 클릭 순서를 넣었다(25건).

⚠️ **이번에 드러난 것 — 다른 화면 수치는 종전 보고와 다르다.** 07-29(3) 감사는
**백엔드를 안 띄운 상태**로 쟀다(그 노트에 "502 상태로 쟀다"고 적혀 있다). 이번엔
백엔드를 띄워서 본문이 실제로 그려졌고, 그 결과 journal 44px미달 29건·기계코드
181건, news 기계코드 172건, home/chart 각 1건이 보인다. **내 변경 전후로 이
숫자들은 동일하다**(stash 후 재측정으로 확인 — 전체 42→31, 감소분은 전부 admin).
즉 기존 결함이 이제 보이는 것이지 회귀가 아니다. **다음 세션 후보 1순위.**

검증: 750 passed / 1 skipped, tsc 0, eslint 0 errors, `next build` 성공,
관리자 2개 화면 44px미달 0 / 무명 0 / 기계코드 0 / 가로스크롤 0.

### 이어서 — 관리자 화면만 앱의 디자인 토큰을 안 쓰고 있었다
"다른 화면이랑 비슷하냐"는 질문에 실제로 스크린샷을 찍어 비교하니 **아니었다.**
계측(44px·기계코드·명암비)은 전부 0인데도 화면이 따로 놀았다 — **자동 계측은
일관성을 못 잰다.** 홈·탐색·보유·분석은 `mone-page-title`(teal 점)·
`mone-page-subtitle`·`mone-home-card`(r=14, slate-900/.5)·`mone-market-tabs`·
`MetricCard`를 공유하는데 관리자만 **하나도 안 쓰고** 전부 raw Tailwind로
직접 그리고 있었다(자체 `Metric` 중복 구현 포함).

실제로 눈에 띈 것:
- **제목이 "관리 / 자 대 / 시보드" 3줄로 깨져 있었다.** 제목 칸에 `min-w-0`가
  없는데 우측에 텍스트 버튼 2개(새로고침·로그아웃)가 절반을 차지해서다.
  배지도 세로로 관/리/자로 눌렸다. → 보유 화면과 같은 **아이콘 버튼 2개**로.
- 루트에 `p-6`가 따로 붙어 관리자 카드만 다른 화면보다 좁았다 → 제거.
- 큰 채움 버튼 3개(teal·dark·emerald)가 나란히 — 이 앱은 큰 채도 버튼을
  안 쓴다(`.mone-primary-action`이 관례). teal과 emerald가 붙어 있어 두 액션이
  구분도 안 됐다 → 은은한 표면 + 주액션 하나만 teal.
- 지표 타일이 모바일에서 **한 줄에 하나씩** 쌓여 화면을 다 먹었다 → 2열.
- 자가보정 탭의 성향/기간이 **라벨 없는 작은 칩 6개**가 한 줄에 섞여 있어,
  앞 3개가 성향이고 뒤 3개가 기간인 걸 눌러보기 전엔 알 수 없었다 →
  탐색 화면과 **같은 라벨(투자성향/투자기간) + 같은 세그먼트**로.
- `github.status === "LOADING"`인데 카드가 **빨간 오류 스타일**이었다(확인 중을
  실패로 오독하게 만든다) → 로딩 중엔 중립.

교훈: 접근성 계측이 0이어도 "같은 앱으로 보이는가"는 별개 축이다. 다음에
화면을 추가하면 `mone-page-title`/`mone-home-card`/`mone-market-tabs`/
`MetricCard`부터 쓸 것 — 색·반경·여백을 손으로 다시 정하지 말 것.

### 이어서 — 누적수익률 -83.89%는 **동시 포지션을 직렬로 센** 결과였다
사용자가 "왜 -80%가 넘냐"고 물어 추적했다. `_portfolio_return_pct`가 체결 건을
**한 건씩 순차로** 자본의 10%씩 걸었다고 보고 630번 복리를 먹이고 있었다.
그런데 실측하면 체결 630건이 **13일에만** 몰려 있고 하루 최대 178건이다 —
같은 날 건은 동시 포지션이지 순차 거래가 아니다. 기하 감쇠를 13번이 아니라
630번 먹인 것이다.

| 기준 | 값 |
|---|---|
| 직렬 10% 슬롯(구) | **-91.9%** (API 표시 -83.89%) |
| 날짜별 등가중 → 거래일 복리(신) | **-39.6%** |
| 거래당 평균 | **-3.94%** |
| 단순 합 | -2484% |

교차점은 **하루 10건**이다(슬롯 0.1 × 10 = 1.0). 그 아래면 두 방식이 거의
같고 위로 갈수록 직렬이 급격히 나빠진다 — 원장은 하루 평균 48건이라 한참
위쪽이었다. (이걸 몰라 처음 쓴 테스트가 "직렬이 항상 더 나쁘다"고 단정했다가
n=10에서 깨졌다. **코드가 아니라 내 테스트가 틀렸다.**)

**같이 나온 것 2개:**
- `all` 분기가 수익률을 `items`에서 뽑고 있었다 — items는 **300건으로 잘리고
  미체결까지 섞여** 있어 `executedTrades`(630)와 **모집단이 달랐다**. 그래서
  API가 -83.89%, 원장 직접 계산이 -91.92%로 갈렸다. 시장별과 같은 체결 집합을
  쓰도록 통일(내부 전달용 `_datedReturns`, 응답에는 안 나간다).
- `avgReturnPct` 키가 아예 없어 화면이 **None**을 받고 있었다.

**해석 주의(화면에 문구로 박았다):** 이 구간은 KOSPI가 6/22 9115 → 7/30 5594로
**-38.6%** 빠졌고, 표본의 **53%가 6/24~6/25 이틀**에 몰려 있다(고점 직후).
07-29(7) 알파 분석의 CAR D+20 **+5.65%** 와 합치면, -39.6%의 상당 부분은
선택 오차가 아니라 **베타**다. 그래서 카드에 거래당 평균·거래일 수를 같이 내고
"건별 복리로 세면 -91.9%로 과대평가된다"를 명시했다.

⚠️ 부수 회귀 하나를 스스로 만들고 잡았다: 상태 칩의 한국어 라벨이 옛 영문 코드보다
길어져 "불러/오는/중" 3줄로 깨졌다. `whitespace-nowrap`+`shrink-0`로 수정(실측 81x26px).
**라벨을 한국어로 바꿀 땐 폭도 같이 볼 것.**
