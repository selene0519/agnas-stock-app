...
def _price_band(score: float, current: float, mode: str, horizon: str, ind: dict | None = None,
    ...
    # 15년 전략 재현(110종목 26,476건, 앱과 같은 ATR 밴드·선착순 청산·왕복비용)에서
    # 국면별 평균손익이 이렇게 나왔다 — **네 가지 국면 정의 전부에서 SIDE가 유일한 음수**:
    #     BEAR +0.547  BULL +0.521  SIDE **-0.256**   (현행 정의)
    #     BEAR +0.596  BULL +0.500  SIDE -0.268       (60일추세 확인)
    #     BEAR +0.380  BULL +0.555  SIDE -0.283       (60일추세 단독)
    # 라이브 clean window(72건)도 같은 말을 한다:
    #     BEAR -3.78  BULL -7.16  SIDE **-10.35**
    # 두 독립 출처가 일치한다. 기계적으로도 납득된다 — 손절이 1.5 ATR로 좁아서
    # **추세 없는 횡보장에서 잡음에 털린다.** 하락장은 오히려 되돌림이 목표를 채운다.
    #
    # 승률은 15년 재현값을 쓴다(표본 26,476건). 이전 값의 출처였던 앙상블
    # 캘리브레이션 표는 라벨이 섞인 원장 위에서 계산된 것이었다.
    _REGIME_WR = {"BULL": 0.427, "SIDE": 0.395, "NEUTRAL": 0.395, "BEAR": 0.459}
    _POOLED_WR = 0.423
    _ev_prob = prob
