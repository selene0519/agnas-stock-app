...
function fallbackSymbol(market: Market): MoneSymbol {
  if (market === "us") {
    return { id: "us-NVDA", symbol: "NVDA", name: "NVIDIA", market: "us", label: "NVIDIA (NVDA)", isWatch: true };
  }
...
export default function ChartPage() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    mone
      .ohlcv({ market: selected.market, symbol: selected.symbol, limit: 260 })
  ...
  return (
    <div className="space-y-6 p-6">
      <div>
        ...
        <p className="mt-1 text-sm text-slate-400">기본으로 보유/추천 종목을 먼저 선택하고 OHLCV 가격 흐름을 확인합니다.</p>
      </div>
        ...
        <div className="rounded-2xl border border-slate-800 bg-slate-900/50 p-5">
          ...
          {!loading && rows.length === 0 && (
            <div className="rounded-xl border border-amber-500/20 bg-amber-500/10 p-8 text-center text-amber-200">
              이 종목의 OHLCV 데이터가 아직 연결되지 않았습니다. data/market/ohlcv 또는 /api/ohlcv 연결을 확인해야 합니다.
            </div>
          )}
