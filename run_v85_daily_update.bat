...
set "LOG=logs\v85_daily_update.log"
echo [INFO] Running MONE v85 daily update... > "%LOG%"
...
%PYEXE% run_v85_daily_update.py >> "%LOG%" 2>&1
set "ERR=%ERRORLEVEL%"
type "%LOG%"
if not "%ERR%"=="0" (
  echo.
  echo [ERROR] MONE v85 update failed. See %LOG%
  pause
  exit /b %ERR%
)
echo [OK] MONE v85 daily update complete.
pause
